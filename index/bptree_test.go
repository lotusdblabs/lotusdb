...
func TestNewBPTree(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"normal", args{opt: BPTreeOptions{DirPath: path, IndexType: BptreeBoltDB, ColumnFamilyName: "test-1", BatchSize: 1000}}, false,
		},
		{
			"no-dir-path", args{opt: BPTreeOptions{DirPath: "", IndexType: BptreeBoltDB, ColumnFamilyName: "test-1", BatchSize: 1000}}, true,
		},
		{
			"no-cf-name", args{opt: BPTreeOptions{DirPath: path, IndexType: BptreeBoltDB, ColumnFamilyName: "", BatchSize: 1000}}, true,
		},
	}
...
func TestBPTree_PutBatch(t *testing.T) {
	path, err := os.MkdirTemp("", "indexer")
	assert.Nil(t, err)
	opts := BPTreeOptions{DirPath: path, IndexType: BptreeBoltDB, ColumnFamilyName: "test", BatchSize: 200000}
	tree, err := NewBPTree(opts)
	assert.Nil(t, err)
	defer func() {
		_ = os.RemoveAll(path)
...
func TestBPTree_DeleteBatch(t *testing.T) {
	path, err := os.MkdirTemp("", "indexer")
	assert.Nil(t, err)
	opts := BPTreeOptions{DirPath: path, IndexType: BptreeBoltDB, ColumnFamilyName: "test"}
	tree, err := NewBPTree(opts)
	assert.Nil(t, err)
	defer func() {
		_ = os.RemoveAll(path)

...
func (ht *HashTable) PutBatch(positions []*KeyPosition, matchKeyFunc ...diskhash.MatchKeyFunc) (int, error) {
	if len(positions) == 0 {
		return 0, nil
	}
...
func (ht *HashTable) DeleteBatch(keys [][]byte, matchKeyFunc ...diskhash.MatchKeyFunc) (int, error) {
	if len(keys) == 0 {
		return 0, nil
	}
